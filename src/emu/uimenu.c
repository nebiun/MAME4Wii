...
static int CLIB_DECL menu_select_game_name_compare(const void *elem1, const void *elem2);
...
static void menu_select_game_populate(running_machine *machine, ui_menu *menu, select_game_state *menustate)
{
	...
	if (menustate->search[0] != 0 || menustate->matchlist[0] == NULL || menustate->rerandomize) {
		...
		if(!menustate->rerandomize)
			qsort(menustate->matchlist,matchcount,sizeof(menustate->matchlist[0]),menu_select_game_name_compare);
	}
	...
	for (curitem = 0; curitem < matchcount; curitem++)
	{
		...
		if (driver != NULL)
		{
			...
			snprintf(tmp_desc,sizeof(tmp_desc),"%s",driver->description);
			if( (s = strchr(tmp_desc,'(')) != NULL)
				*s = '\0';
			if( (s = strchr(tmp_desc,'/')) != NULL)
				*s = '\0';

			ui_menu_item_append(menu, tmp_desc, driver->name, (cloneof == NULL || (cloneof->flags & GAME_IS_BIOS_ROOT)) ? 0 : MENU_FLAG_INVERT, (void *)driver);
		}
	}
...
#else
	sprintf(&tempbuf[0][0], "Choose the game: ");
#endif
