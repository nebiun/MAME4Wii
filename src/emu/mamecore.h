...
#else
#define GIANT_STRING_BUFFER_SIZE		8192
#endif
...
#define fatalerror(...)					real_fatalerror(__FILE__,__LINE__, NULL, MAMERR_FATALERROR, __VA_ARGS__)
#define fatalerror_exitcode(m,e,...)	real_fatalerror(__FILE__,__LINE__, m, e, __VA_ARGS__)
...
DECL_NORETURN void CLIB_DECL real_fatalerror(const char *src, int line, running_machine *machine, int exitcode, const char *text, ...) ATTR_PRINTF(5,6) ATTR_NORETURN;
