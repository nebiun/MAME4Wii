...
#ifdef __WII__
			wii_debug("%s: return from setjmp: %d\n",__FUNCTION__,error);
#endif			
		}
...
void mame_printf_warning(const char *format, ...)
{
	...
	{
#ifdef __WII__
		output_cb[OUTPUT_CHANNEL_WARNING] = mame_null_output_callback;
#else
...
void CLIB_DECL real_fatalerror(const char *src, int line, running_machine *machine, int exitcode, const char *text, ...)
{
	va_list arg;
	char *p = giant_string_buffer;
	int len;
	
	sprintf(p, "%s(%d):", src, line);
	len = strlen(p);
	p += len;
	
	/* dump to the buffer; assume no one writes >2k lines this way */
	va_start(arg, text);
	vsnprintf(p, GIANT_STRING_BUFFER_SIZE - len, text, arg);
	va_end(arg);
...
static running_machine *create_machine(const game_driver *driver)
{
	...
	if (machine->config->driver_data_size != 0)
	{
		...
		if (machine->driver_data == NULL) {
			wii_debug("%s: cannot alloc driver data (%d bytes)\n", __FUNCTION__, machine->config->driver_data_size);
			goto error;
		}
