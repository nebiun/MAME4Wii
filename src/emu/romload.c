...
***************************************************************************/
#ifdef __WII__
#define TEMPBUFFER_MAX_SIZE		(10 * 1024 * 1024)
#else
...
static void display_loading_rom_message(rom_load_data *romdata, const char *name)
{
	...
	if (name != NULL) {
		sprintf(buffer, "Loading %d%% of %d", 
			(UINT32)(100 * (UINT64)romdata->romsloadedsize / (UINT64)romdata->romstotalsize),
			 romdata->romstotalsize);
	}
	else {
		sprintf(buffer, "Loading Complete (%d)", romdata->romstotalsize);
	}
