...
#ifdef __WII__
	if (driver_list_get_count(drivers) > 500 && !clone_of && is_clone)
#else
...
int mame_validitychecks(const game_driver *curdriver)
{
	...
	*(UINT8 *)&lsbtest = 0xff;
	...
	for (drivnum = 0; drivers[drivnum]; drivnum++)
	{
		...
		if(validate_driver(drivnum, config, &tables) != 0) {
			error |= 010;
		}
		driver_checks += get_profile_ticks();
		...
		if(validate_roms(drivnum, config, &rgninfo) != 0) {
			error |= 020;
		}
		rom_checks += get_profile_ticks();
		...
		if(validate_inputs(drivnum, config, tables.defstr, &portlist) != 0) {
			error |= 040;
		}	
		input_checks += get_profile_ticks();
		...
		if(validate_cpu(drivnum, config) != 0) {
			error |= 0100;	
		}
		cpu_checks += get_profile_ticks();
		...
		if(validate_display(drivnum, config) != 0) {
			error |= 0200;
		}
		display_checks += get_profile_ticks();
		...
		if(validate_gfx(drivnum, config, &rgninfo) != 0) {
			error |= 0400;
		}
		gfx_checks += get_profile_ticks();
		...
		if(validate_sound(drivnum, config) != 0) {
			error |= 01000;
		}
		sound_checks += get_profile_ticks();
		...
		if(validate_devices(drivnum, config, portlist, &rgninfo) != 0) {
			error |= 02000;
		}
		device_checks += get_profile_ticks();
