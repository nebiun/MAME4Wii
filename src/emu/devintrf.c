...
device_config *device_list_add(device_config **listheadptr, const device_config *owner, device_type type, const char *tag, UINT32 clock)
{
	...
	/* tag len must be multiple of sizeof(int) to avoid device->inline_config spare address */
	tag_len = strlen(tag) + 1;
	if(tag_len % sizeof(int))
		tag_len += (sizeof(int) -  (tag_len % sizeof(int)));
	
	/* get the size of the inline config */
	configlen = (UINT32)devtype_get_info_int(type, DEVINFO_INT_INLINE_CONFIG_BYTES);

	/* allocate a new device */
	device = (device_config *)alloc_array_or_die(UINT8, sizeof(*device) + tag_len + configlen);
	...
	device->inline_config = (configlen == 0) ? NULL : (device->tag + tag_len);
