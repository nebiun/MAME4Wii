//============================================================
//
//  wiiwork.c - OSD core work item functions
//
//  Copyright (c) 1996-2009, Nicola Salmoria and the MAME Team.
//  Visit http://mamedev.org for licensing and usage restrictions.
//
//  based on SDLMAME by Olivier Galibert and R. Belmont
//
//============================================================
#include "osdcore.h"
#include "osinline.h"
...
osd_work_queue *osd_work_queue_alloc(int flags)
{
	...
	queue = (osd_work_queue *)wii_calloc(sizeof(*queue));
	if (queue == NULL)
		goto error;
...
#ifdef WIIWORK_DEBUG
	wii_debug("%s: queue->threads %d\n",__FUNCTION__,queue->threads);
#endif
	
	// allocate memory for thread array (+1 to count the calling thread)
	queue->thread = (work_thread_info *)wii_calloc((queue->threads + 1) * sizeof(queue->thread[0]));
...
osd_work_item *osd_work_item_queue_multiple(osd_work_queue *queue, osd_work_callback callback, INT32 numitems, void *parambase, INT32 paramstep, UINT32 flags)
{
	...
	for (itemnum = 0; itemnum < numitems; itemnum++) {
		...
		if (item == NULL) {
			// allocate the item
			item = (osd_work_item *)wii_calloc(sizeof(*item));
			if (item == NULL)
				return NULL;
			item->event = NULL;
			item->queue = queue;
